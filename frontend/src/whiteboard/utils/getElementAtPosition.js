...
const onLine = ({x1, y1, x2, y2, x, y, maxDist = 1}) => {
    const a = {x: x1, y: y1};
    const b = {x: x2, y: y2};
    const c = {x, y};

    const offset = distance(a, b) - distance(a, c) - distance(b, c);
    return Math.abs(offset) < maxDist ? cursorPositions.INSIDE: null;
}
...
const positionWithinElement = (x, y, element) => {
    ...
    switch(type) {
        ...
        case toolTypes.ELLIPSE:
            ...
            //lastly checking whether i am inside
        ...
        case toolTypes.PENCIL:
            const betweenAnyPoint = element.points.some((point, index) => {
                ...
                return onLine({x1:point.x, y1:point.y, x2:nextPoint.x, y2:nextPoint.y, x, y, maxDist: 5,});
...
export const getElementAtPosition = (x,y,elements) => {
    ...
    //finds the first element at which cursor is
